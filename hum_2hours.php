...
if(!file_exists('pic/hum_2hours.png'))
       {
        $myData = new pData();
        $sql =<<<EOF
...
EOF;
        ...
        while($row = $ret->fetchArray(SQLITE3_ASSOC))
          {
            $time=round($row['ID']/600)*600;
      			$time_h2[]=date("H:i",$time);
            $hum2[] = $row['HUM'];
        }
        $myData->addPoints($time_h2,"Timestamp");#添加坐标信息
        $myData->setAbscissa("Timestamp");

        $myData->addPoints($hum2,"Data");#添加数据
        ...
        $Config = array("R"=>0,"G"=>0,"B"=>0,"ForceTransparency"=>50, "AroundZero"=>1);
        $myPicture->drawFilledSplineChart($Config);


        $myPicture->render("pic/hum_2hours.png");
      }
?>
